...
/// Execute a compositor command synchronously (for use from non-async contexts).
pub fn execute_command_sync(cmd: CompositorCommand) -> Result<()> {
    match cmd {
...
pub async fn run_listener(tx: &mpsc::Sender<ServiceEvent<Compositor>>) -> Result<()> {
    ...
    // We need to wrap the mpsc sender in Arc for sharing across handlers
