//! D-Bus proxy definitions and StatusNotifierWatcher implementation for system tray.
...
impl StatusNotifierWatcher {
    ...
    pub async fn start_server() -> anyhow::Result<Connection> {
                ...
                tokio::select! {
                    ...
                    _ = interval.tick() => {
                        if let Err(e) = Self::discover_items(&internal_connection, &internal_interface).await {
                            debug!("Failed to discover tray items: {e}");
                        }
                    }
                }
            }
        ...
        if let Err(e) = Self::discover_items(&connection, &interface).await {
            debug!("Failed initial tray item discovery: {e}");
        }
...
/// StatusNotifierItem D-Bus proxy.
...
pub struct MenuLayout(pub i32, pub MenuLayoutProps, pub Vec<MenuLayout>);

impl<'a> serde::Deserialize<'a> for MenuLayout {
    fn deserialize<D: serde::Deserializer<'a>>(
        deserializer: D,
    ) -> std::result::Result<Self, D::Error> {
        let (id, props, children) =
            <(i32, MenuLayoutProps, Vec<(zvariant::Signature, Self)>)>::deserialize(deserializer)?;
        Ok(Self(id, props, children.into_iter().map(|x| x.1).collect()))
    }
}

/// Menu layout properties.
#[derive(Clone, Debug, Default, Type, zvariant::DeserializeDict)]
