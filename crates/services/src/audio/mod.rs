...
///
/// Connection failures are surfaced through `ServiceStatus::Error` and retried,
/// so a missing or late-starting PulseAudio/PipeWire never panics the service.
/// The libpulse mainloop is blocking and `!Send`, hence the dedicated thread.
fn start_listener(data: Mutable<AudioData>, status: Mutable<ServiceStatus>) {
    let run_status = status.clone();
    crate::listener::spawn_blocking_listener("audio", status, move || {
        run_pulse_loop(&data, &run_status)
    });
}

/// Connect to PulseAudio and process events until the connection fails.
fn run_pulse_loop(
    data: &Mutable<AudioData>,
    status: &Mutable<ServiceStatus>,
) -> anyhow::Result<()> {
    let mut proplist =
        Proplist::new().ok_or_else(|| anyhow::anyhow!("Failed to create PulseAudio proplist"))?;
    ...
    context
        ...
        .map_err(|e| anyhow::anyhow!("Failed to connect to PulseAudio: {e}"))?;

    // Wait for context to be ready
    loop {
        match mainloop.iterate(true) {
            IterateResult::Quit(_) | IterateResult::Err(_) => {
                anyhow::bail!("PulseAudio mainloop error during connect");
            }
    ...
    *status.lock_mut() = ServiceStatus::Active;
    ...
    while pending_queries.get() > 0 {
        match mainloop.iterate(false) {
            IterateResult::Quit(_) | IterateResult::Err(_) => {
                anyhow::bail!("PulseAudio mainloop error during initial query");
            }
    ...
    // Update shared state with initial data
    {
        ...
        debug!(
            ...
            current.sink_volume, current.sink_muted, current.source_volume, current.source_muted
        );
    ...
    loop {
        ...
        match mainloop.iterate(!has_pending) {
            IterateResult::Quit(_) | IterateResult::Err(_) => {
                anyhow::bail!("PulseAudio mainloop error in event loop");
            }
            IterateResult::Success(_) => {
