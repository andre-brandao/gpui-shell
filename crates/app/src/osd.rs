...
impl OsdView {
    ...
    fn render_horizontal(&self, cx: &Context<Self>) -> impl IntoElement {
        ...
        let icon_color = if muted { status.error } else { colors.text };
...
pub fn open(services: Services, position: OsdPosition, cx: &mut App) {
