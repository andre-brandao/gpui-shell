...
/// Initialize Zed's theme system with default settings.
pub fn init_theme(cx: &mut App) {
    // Initialize settings store first (required by theme)
    settings::init(cx);

    // Initialize theme with just the base/fallback theme (no extension loading)
